...
class CommandsManager extends events_1.EventEmitter {
     ...
     */
    constructor(client, directory, loadAll, options) {
        super();
        if (!client)
    ...
    async loadAll() {
        ...
        const cmdsPath = await readDirFiles_1.readDirAndPush(baseDir);
        for (const cmdPath of cmdsPath) {
            const cmdImport = await Promise.resolve().then(() => require(cmdPath));
     ...
     */
    async registerAllApplicationCommands(commands = this.commands) {
        if (!commands)
            throw new Error("Commands not found");
