...
async function run(client, message) {
    ...
    const command = client.commands.message.get(commandName) ||
        client.commands.message.find((cmd) => cmd.aliases && cmd.aliases.includes(commandName));
    if (!command)
        return;
    if (command.before)
        await command.before(message, args);
    /**
     * Handle inhibitors
     */
    const inhibitors = client.inhibitors?.filter((i) => i.type.includes("MESSAGE_COMMAND") || i.type.includes("ALL"));
    if (inhibitors && inhibitors.size) {
        const sorted = [...inhibitors.values()].sort((a, b) => b.priority - a.priority);
        for (const i of sorted) {
            if (!i.execute(client, message))
                return i.onFailure(client, message);
        }
    }
    /* ---------------PERMISSIONS--------------- */
    if (command.userPermissions.includes("BOT_ADMIN") &&
        !client.admins?.includes(message.author.id))
        return client.handlers.messageCommands?.emit("userMissingPermissions", message, "BOT_ADMIN");
    /* ---------------IN-GUILD--------------- */
    if (message.guild) {
        if (command.only === "DM")
            return;
        ...
        if (command.userPermissions.length) {
            for (const permission of command.userPermissions) {
                if (!member.permissions.has(permission))
                    return client.handlers.messageCommands?.emit("userMissingPermissions", message, permission);
            }
        }
        if (command.clientPermissions.length) {
            for (const permission of command.clientPermissions) {
                if (!message.guild.me.permissions.has(permission))
                    return client.handlers.messageCommands?.emit("clientMissingPermissions", message, permission);
            }
        }
    }
    else {
        /* ---------------IN-DM--------------- */
        if (command.only === "GUILD")
            return;
    }
    /* ---------------COOLDOWNS--------------- */
    if (!client.admins?.includes(message.author.id)) {
        if (!command.cooldowns.has(command.name)) {
            command.cooldowns.set(command.name, new collection_data_1.Collection());
        }
